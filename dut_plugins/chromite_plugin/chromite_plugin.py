...
class ChromitePlugin(object):
    ...
    def compile(self, module_dir, asm_dir, asm_gen):
        ...
        pytest.main([pytest_file, '-n={0}'.format(self.jobs), '-k={0}'.format(self.filter), '--html={0}/compile.html'.format(self.compile_output_path), '--self-contained-html', '--asm_dir={0}'.format(asm_dir), '--yaml_config={0}'.format(self.yaml_config), '--gen_suite={0}'.format(asm_gen), '--log-cli-level=DEBUG'])
