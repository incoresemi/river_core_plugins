...
class chromite(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config is None:
            print("Please enter chromitebin path in configuration.")
            raise SystemExit
        else:
            self.chromitebin = config['chromitebin']
    ...
    def runTests(self, testList):
        ...
        for file in testList:
            ...
            d = dict(elf=elf,
                     testDir=test_dir,
                     isa=self.isa,
                     chromitebin=self.chromitebin)

            command = Template(
                '''ln -sf ${chromitebin}/* ${testDir}/. ; elf2hex 8 4194304 $elf 2147483648 > ${testDir}/code.mem'''
            ).safe_substitute(d)

            execute+=command+';'

            command = "cd " + testentry['work_dir'] + '; timeout 30s ./chromite_core +rtldump'
            execute += command+';'
