...
class cclass_verilator_plugin(object):
    '''
        Plugin to set cclass as the target
    '''

    @dut_hookimpl
    def init(self, ini_config, test_list, work_dir, coverage_config,
             plugin_path):
        self.name = 'cclass_verilator'
        ...
        if coverage_config:
            logger.info(
                "Coverage is enabled, compiling the cclass with coverage")
            verilator_command = verilator_command.format('--coverage-line')
        else:
            logger.info(
                "Coverage is disabled, compiling the cclass with usual options"
            )
    ...
    def build(self):
        ...
        for test, attr in self.test_list.items():
            ...
            sim_setup = 'ln -f -s ' + self.sim_path + '/out . && '
            sim_setup += 'ln -f -s ' + self.plugin_path + self.name +\
                    '_plugin/boot/boot.* . && '
    ...
    def run(self, module_dir):
        ...
        pytest_file = module_dir + '/cclass_verilator_plugin/gen_framework.py'
