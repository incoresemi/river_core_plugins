...
class chromite_cadence_plugin(object):
    ...
    def init(self, ini_config, test_list, work_dir, coverage_config,
        ...
        for key, value in self.test_list.items():
            ...
            if ('utg' in set(generators)) and (len(set(generators)) > 1):
                logger.critical(
                    'Running tests generated by utg with other tests generated by other generators is \'currently\' not supported'
                )
                logger.critical(
                    'Generate and run the tests from utg separately')
                try:
