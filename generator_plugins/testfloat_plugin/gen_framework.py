...
def create_asm(gen_file,parameter_list,gen_cmd):
    ...
    with open(assembly_file, 'w+') as asm_file_pointer:
        ...
        for case_index in range(0, 3000):
                    ...
                    for rounding_mode_index in range(0, len(rounding_mode)):
                        ...
                        for cnt_index in range(int(count)):
                            ...
                            for num_index in range(int(num_tests)):
                                ...
                                combine = ['{0} -seed {1} -n {2} {3} {4}'.format(
                                        testfloat_bin, gen_seed, tests_per_instruction,rounding_mode_gen, gen_inst), param_list, testdir + test_prefix + '.gen', testdir]
...
def test_input(request):
    ...
    (ret, out, err) = utils.sys_command_file(generation_param[0], generation_param[2])
