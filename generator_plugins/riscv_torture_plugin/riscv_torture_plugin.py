...
class riscv_torture_plugin(object):
    ...
    def pre_gen(self, spec_config, output_dir):
        ...
        if not os.path.isdir(f'{self.output_dir}/riscv-torture/'):
            ...
            subprocess.call(shlex.split(f"java -Xmx1G -Xss8M -XX:MaxPermSize=128M -jar sbt-launch.jar 'generator/run'"))
    ...
    def gen(self, module_dir, output_dir):
        ...
        if not result.stdout:
            logger.debug(f'applying patch {module_dir}/{self.name}_plugin/patch')
        ...
        asm_stats_list = glob.glob(asm_dir + '**/*.stats')
        ...
        for test in  asm_test_list:
            ...
            test_list[base_key]['cc_args'] = ' -mcmodel=medany -static -std=gnu99 -O2 -fno-common -fno-builtin-printf -fvisibility=hidden '
